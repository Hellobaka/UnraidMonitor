...
using me.cqp.luohuaming.UnraidMonitor.Core;
using me.cqp.luohuaming.UnraidMonitor.Core.Domain;
...
namespace me.cqp.luohuaming.UnraidMonitor.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.UnraidMonitor";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.UnraidMonitor", 1, 9, "Unraid监控", "1.0.0", 1, "落花茗", "Unraid监控插件，支持自定义指令与回复内容", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.UnraidMonitor", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.UnraidMonitor", AppData.CQLog);	
